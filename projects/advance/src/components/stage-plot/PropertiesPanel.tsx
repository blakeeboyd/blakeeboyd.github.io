...
export function PropertiesPanel() {
  ...
  return (
    <div className="adv-stage-panel__section">
      <h3 className="adv-stage-panel__title">Properties</h3>
      <div className="adv-props">
        <div className="adv-props__row">
          <span className="adv-props__label">Label</span>
          <input
            className="adv-props__input"
            value={el.label}
            onChange={(e) => updateElement(el.id, { label: e.target.value })}
          />
        </div>
        <div className="adv-props__row">
          <span className="adv-props__label">X</span>
          <input
            className="adv-props__input"
            type="number"
            ...
            style={{ width: 60 }}
          />
          <span className="adv-props__label">Y</span>
          <input
            className="adv-props__input"
            type="number"
        ...
        <div className="adv-props__row">
          <span className="adv-props__label">W</span>
          <input
            className="adv-props__input"
            type="number"
            value={el.width}
            ...
            style={{ width: 60 }}
          />
          <span className="adv-props__label">H</span>
          <input
            className="adv-props__input"
            type="number"
            value={el.height}
        ...
        <div className="adv-props__row">
          <span className="adv-props__label">Rotate</span>
          <input
            className="adv-props__input"
            type="number"
            min={0}
            ...
            style={{ width: 60 }}
          />
          <span style={{ fontSize: '0.7rem', color: 'var(--adv-text-tertiary)' }}>deg</span>
        </div>
        <div className="adv-props__row">
          <span className="adv-props__label">Color</span>
          <input
            type="color"
            value={el.color}
        ...
        <div className="adv-props__row">
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.75rem', color: 'var(--adv-text-secondary)', cursor: 'pointer' }}>
            <input
              type="checkbox"
              checked={el.labelVisible}
              onChange={(e) => updateElement(el.id, { labelVisible: e.target.checked })}
              style={{ accentColor: 'var(--adv-accent)' }}
            />
            Show label
          </label>
        </div>
        <div className="adv-props__row" style={{ marginTop: 8, gap: 6 }}>
          <button className="adv-btn adv-btn--sm" onClick={() => duplicateElement(el.id)}>Duplicate</button>
          <button className="adv-btn adv-btn--sm adv-btn--danger" onClick={() => { removeElement(el.id); selectElement(null); }}>Delete</button>
        </div>
      </div>
    </div>
  );
