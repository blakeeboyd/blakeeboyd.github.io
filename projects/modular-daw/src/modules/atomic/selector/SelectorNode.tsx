import { Handle, Position } from '@xyflow/react';

export function SelectorNode() {
  return (
    <div className="daw-node daw-node--atomic" style={{ minWidth: 60 }}>
      <Handle type="target" position={Position.Left} id="a"
        className="daw-handle daw-handle--audio daw-handle--pos-1of3" />
      <Handle type="target" position={Position.Left} id="b"
        className="daw-handle daw-handle--audio daw-handle--pos-2of3" />
      <Handle type="target" position={Position.Bottom} id="ctrl"
        className="daw-handle daw-handle--audio daw-handle--pos-3of3" />
      <div className="daw-node__header"><span>Select</span></div>
      <div className="daw-node__body daw-node__symbol">A|B</div>
      <Handle type="source" position={Position.Right} id="out"
        className="daw-handle daw-handle--audio" />
    </div>
  );
}
