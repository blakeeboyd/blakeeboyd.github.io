...
- **Archives**: fourth section of the admin panel. Lists every entry under `backchannel/archives` sorted most-recent first. Each row has three actions: `view` opens a standalone HTML transcript in a new tab via a Blob URL (the viewer is its own document with embedded styles, dark-mode aware via `prefers-color-scheme`), `json` downloads a `backchannel-archive-{iso}.json` file, and `delete` permanently removes the archive. The metadata listing is live via `listenArchives` (refreshes when archives are created or deleted elsewhere); the full messages payload is fetched on demand via `getArchive(id)` rather than held in memory.
