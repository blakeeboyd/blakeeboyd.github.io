...
- **Pin/unpin messages**: long-press a message bubble (touch) or right-click it (mouse) while admin to toggle its pinned state. Pins live at `backchannel/{room}/pinned/{msgId}` and are mirrored to every client via `listenPinned`. Pinned bubbles get a `.msg--pinned` accent (accent-color border + small accent dot in the corner). The top-bar pin icon (`#pin-toggle`) appears for everyone — admin and student — once any pin exists; clicking it opens the `.pin-panel` popover, which lists every pinned message with the original name/time/text, a `jump` button that scrolls to the original bubble (when it's still within the 200-message window), and an `unpin` button (admin only). Soft-capped at `PIN_CAP` (50) — beyond that the pin action shows an alert.
